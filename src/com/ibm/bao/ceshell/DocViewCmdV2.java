 ...
 * @date   Oct 3, 2010
 */
public class DocViewCmdV2 extends BaseCommand {
	...
	private void fetchDocContents(
		...
		for (int i = 0; i < totalElements; i++) {
			...
			File nextOutputFile = null;

			if (retrievalName == null) {
				retrievalName = doc.get_Id().toString() + ".bin";
			}
			//nextOutputFile = nameMgr.createFile(retrievalName);
			nextOutputFile = names[i];
		...
		// no content elements
		if (pos == 0) {
			getResponse().printOut("\tno content elements for doc with id " + 
					doc.get_Id().toString());
		}
	}
